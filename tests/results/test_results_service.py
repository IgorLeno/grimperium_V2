...
def test_compare_runs_returns_basic_metric_rows(
    ...
    assert [row.property_id for row in comparison.rows] == [
        "standard_enthalpy_of_formation",
        "standard_enthalpy_of_formation",
    ]
    assert [row.analysis_mode for row in comparison.rows] == [
...
def test_compare_runs_rejects_incompatible_property_ids(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("GRIMPERIUM_RUNS_DIR", str(tmp_path / "runs"))
    run_service = RunService.from_environment()
    run_ids: list[str] = []
    for property_id in ("standard_enthalpy_of_formation", "other_property"):
        manifest = run_service.create_run(
            property_id=property_id,
            method_id="pm7_delta_learning",
            method_version="0.1.0",
            method_snapshot={"display_name": property_id},
            execution_overrides={},
            dataset_ref=None,
        ...
        output_path.parent.mkdir(parents=True, exist_ok=True)
        pd.DataFrame(
            {"mol_id": ["m1"], "H298_cbs": [-100.0], "H298_predicted": [-101.0]}
        ).to_csv(output_path, index=False)
        run_service.start_run(manifest.run_id)
        run_service.attach_output_paths(manifest.run_id, {"results_csv": output_path})
        run_service.complete_run(manifest.run_id, success_count=1, failure_count=0)
    ...
    with pytest.raises(ValueError, match="property_id"):
        ResultsService(run_service=run_service).compare_runs(run_ids)
