...
from grimperium.core.csv_data_loader import ValidationError, ValidationReport
...
class TestBatchOrchestratorRun:
    ...
    def test_callback_is_called(self, orchestrator_with_csv):
        ...
        # Callback should be called (2 pending molecules)
        # But in dry_run mode, callback is only called during actual processing
        # So we test without dry_run
...
class TestBatchOrchestratorVisibility:
    ...
    def test_prints_validation_report_permissive(
        self, tmp_path, capsys
    ):
        """v2.2: Prints validation warnings to console in permissive mode."""
