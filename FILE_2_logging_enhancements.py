...
def log_rdkit_done(logger: logging.Logger, mol_id: str, **properties) -> None:
    """
    ...
    """
    props_str = ", ".join(f"{k}={v:.2f}" if isinstance(v, float) else f"{k}={v}" 
...
def log_crest_done(logger: logging.Logger, mol_id: str, num_conformers: int, time_seconds: float) -> None:
    """
    ...
    """
    logger.info(f"[{mol_id}]   ✓ Generated {num_conformers} conformers in {time_seconds:.1f}s")
...
def log_mopac_conformer_done(logger: logging.Logger, mol_id: str, conf_idx: int, 
                             delta_energy: float, time_seconds: float) -> None:
    """
    ...
    """
    logger.info(f"[{mol_id}]   • Conformer {conf_idx}: ✓ ΔE={delta_energy:.2f} kcal/mol ({time_seconds:.1f}s)")


def log_mopac_done(logger: logging.Logger, mol_id: str, best_conformer_idx: int, 
                   best_delta_energy: float, time_seconds: float) -> None:
    """
    ...
    """
    logger.info(f"[{mol_id}]   ✓ Selected conformer #{best_conformer_idx} with ΔE={best_delta_energy:.2f} kcal/mol")


def log_batch_summary(logger: logging.Logger, batch_id: str, total: int, 
                      success: int, failed: int, skipped: int = 0) -> None:
    """
...
@contextmanager
def log_phase(logger: logging.Logger, phase_name: str, mol_id: str = ""):
    """
...
class LogMessages:
    ...
    @staticmethod
    def rdkit_descriptor(mol_id: str, **props) -> str:
