...
class TemporaryDirectoryManager:
    ...
    @classmethod
    def get_mopac_temp_files(
        cls, batch_id: str, mol_id: str, conformer_idx: int
    ) -> dict[str, Path]:
        """
    ...
    def cleanup_old_batches(cls, keep_last_n: int = 5) -> None:
        ...
        batch_dirs = sorted(
            cls.ROOT_TEMP.glob("batch_*"), key=lambda p: p.stat().st_mtime, reverse=True
        )
    ...
    @classmethod
    def print_temp_structure(cls, root_dir: Path | None = None) -> None:
        """
        ...
        for batch_dir in sorted(root_dir.glob("batch_*")):
            ...
            total_size_mb = sum(
                f.stat().st_size for f in batch_dir.rglob("*") if f.is_file()
            ) / (1024**2)
...
def get_mopac_temp_files(
    batch_id: str, mol_id: str, conformer_idx: int
) -> dict[str, Path]:
    """Get MOPAC-specific temporary file paths for a conformer."""
    return TemporaryDirectoryManager.get_mopac_temp_files(
        batch_id, mol_id, conformer_idx
    )
...
def print_temp_structure(root_dir: Path | None = None) -> None:
    """Print temporary directory structure for debugging."""
