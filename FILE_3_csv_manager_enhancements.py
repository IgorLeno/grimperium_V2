...
class DeltaCalculations:
    ...
    @staticmethod
    def calculate_deltas_and_select(mopac_hof_values: List[float]) -> Tuple[float, float, float, int]:
        """
        ...
        best_idx = np.argmin(values)  # Index in original array
...
class CSVManagerExtensions:
    ...
    def update_molecule_with_mopac_results(
        ...
        try:
            ...
            logger.info(f"[{mol_id}] Updated CSV with calculated deltas (δ1={delta_1:.2f}, δ2={delta_2:.2f}, δ3={delta_3:.2f})")
            return True
